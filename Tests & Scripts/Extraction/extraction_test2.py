...
#Note: Script expects "YOUR_OUTPUT" to be in any of the following format (i.e. (tweet [tab] extracted entities [tab] disambiguations)  or (tweet [tab] extracted entities))
# Example ----- Spotted : Kanye West Celebrates LAMB With Gwen Stefani : New York Fashion Week is coming to a close , but not before ... http://bit.ly/cSyZUi   Kanye West||New York Fashion Week||Gwen Stefani	{'Kanye West': '/en/kanye_west', 'New York Fashion Week': '/en/new_york_fashion_week', 'Gwen Stefani': '/en/gwen_stefani'}
# Example ----- Spotted : Kanye West Celebrates LAMB With Gwen Stefani : New York Fashion Week is coming to a close , but not before ... http://bit.ly/cSyZUi   Kanye West||New York Fashion Week||Gwen Stefani
import jellyfish
...
for l1 in out1:
    ...
    predicted = s1.split('\t')
    actual = s2.split('\t')
    predicted_entities = predicted[1].split('||')
